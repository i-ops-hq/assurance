...
`assurance_cli.__version__` was told the wrong thing. Reported by an outside reviewer 2026-08-29.
...
def test_version_agrees_with_pyproject() -> None:
    assert assurance_cli.__version__ == _declared()


def test_version_is_not_a_placeholder() -> None:
    assert assurance_cli.__version__ != "0.0.0+unknown"
