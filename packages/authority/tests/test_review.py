...
def test_an_unrecognised_clearance_field_is_refused_not_ignored() -> None:
    """The worst defect this package has had, reported by an outside tester on 2026-09-03.

    A file written with `may_see` was accepted, the clearance defaulted to empty, and every task was
    refused **with a reason naming labels the author had just granted**. The tool stated something
    false about the reader's own file — the exact failure this family exists to refuse.
    """
    with pytest.raises(DeclarationError) as exc:
        loads(json.dumps({
            "principals": [{"id": "tom", "may_see": ["design-partner-pipeline"]}],
            "tasks": [{"name": "t", "initiator": "tom", "requires": ["design-partner-pipeline"]}],
        }))

    assert "may_see" in str(exc.value)
    assert "may_receive" in str(exc.value)


def test_an_omitted_clearance_is_not_an_empty_one() -> None:
    """A principal with no `may_receive` is under-declared, not cleared for nothing."""
    with pytest.raises(DeclarationError) as exc:
        loads(json.dumps({
            "principals": [{"id": "tom"}],
            "tasks": [{"name": "t", "initiator": "tom", "requires": ["x"]}],
        }))

    assert "may_receive" in str(exc.value)


def test_every_missing_task_field_is_named_at_once() -> None:
    """Reported as "schema whack-a-mole": one missing field per run is three runs to learn a shape."""
    with pytest.raises(DeclarationError) as exc:
        loads(json.dumps({
            "principals": [{"id": "tom", "may_receive": ["x"]}],
            "tasks": [{}],
        }))

    message = str(exc.value)
    assert "'name'" in message and "'initiator'" in message and "'requires'" in message


def test_an_unrecognised_task_field_is_refused() -> None:
    with pytest.raises(DeclarationError) as exc:
        loads(json.dumps({
            "principals": [{"id": "tom", "may_receive": ["x"]}],
            "tasks": [{"name": "t", "initiator": "tom", "requires": ["x"], "priority": "high"}],
        }))
