...
def test_a_refusal_shows_its_reason_without_asking_for_json(tmp_path: Path, capsys) -> None:
    """A tester saw three bare `refuse` rows and had no way to see why.

    The sentence naming what each person may not receive was sitting in a field he had no reason to
    look at. A refusal without its reason is what makes somebody widen every grant they can find.
    """
    main([_write(tmp_path, DECLARATION)])
